...
if ( $layout_type == 'latest' ) {
    ...
    $city_name = get_post_meta(get_the_ID(),'city',true);
    if(isset($city_name) && $city_name != ""){
            $meta_html['date'] .= '<span class="w-blog-post-meta-map_city_name">' . $city_name . '</span><span class="seprater">|</span>'.get_the_date( 'm' ).'/'.get_the_date( 'd' ).'/'.get_the_date( 'Y' );
    }
    else{
        //$meta_html['date'] .= get_the_date();
        $meta_html['date'] .= get_the_date( 'm' ).'/'.get_the_date( 'd' ).'/'.get_the_date( 'Y' );
    }
}
$meta_html['date'] .= '</time>';
...
<article <?php post_class( $post_classes ) ?> data-id="<?php the_ID() ?>" data-categories="<?php echo implode( ',', $categories_slugs ) ?>">
	...
	<?php //echo "count".get_post_meta(get_the_ID(),'wpb_post_views_count',true); ?>
	<div class="w-blog-post-h">
		...
		<?php
			if ( $has_preview AND empty( $featured_html ) ): ?>
			<a href="<?php echo $link; ?>"<?php echo $anchor_atts ?>>
				<div class="w-blog-post-preview">
					<div class="thumbnail-link__thumb" style="background-size: cover; background-image: url('<?php echo $thumb; ?>');"></div>
					<span class="w-blog-post-preview-icon" style="padding-bottom: <?php echo $us_blog_img_ratio; ?>%;"></span>
				</div>
			</a>
		<?php endif/*( empty( $featured_html ) )*/; ?>
...
<?php /* } */
else/*if ( $use_special_quote_markup )*/: ?>
