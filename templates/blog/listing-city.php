...
{
    if(isset($_GET['orderby']) && $_GET['orderby'] != "" && isset($_GET['city_name']) && $_GET['city_name'] != "")
    {
        if($_GET['orderby'] == "most_popular")
        {
            $query_args = array();
        ...
        else if($_GET['orderby'] == "most_recent")
        {
            $query_args = array();
			$query_args['meta_key'] = 'city';
...
if ( $wp_query->max_num_pages > 1 ) {
	...
	$json_data = array(
		...
		'template_vars' => array(
			...
			'nipl_city' => $_GET['city_name'],
			'nipl_filter' => $_GET['orderby']
		),
	);
	if ( $pagination != 'none' ) {
	?>
